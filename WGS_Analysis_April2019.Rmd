...
# Data import
Three groups of data sets to import: 1) WGS Kraken empirical data; 2) sample metadata; 3) amplicon data. The amplicon data are represented by 6 data files: two markers (ITS2 and *rbcL*) and three levels of taxonomic matching (species, genus, and family). We later format the Kraken data into three datasets representing those taxonomic levels.
...
*again, the Kraken simulations have been removed from the analysis because they were not adding any particular value*
...
1. Combine amplicon and empirical Kraken data
...
1. remove reads below the isolation / PCR negative control thresholds
      ...
      a) ideally, would base these on negative controls **for a given Illumina run** (two separate runs here, at UGA and Emory Genome center; we will need to keep those separate); but **unfortunately** there are not negative controls for the Emory Genome Center data; use the UGA thresholds for all the data
...
1. remove negative control rows (once the above is completed)
...
# Clean up
rm(maxy, all.indexy_family, all.indexy_genus, all.indexy_species)

```

## match Kraken data to sample data; false negative and quantitative analyses

1. Create a qualitative variable that equals 1 if the proportion is greater than 0, and 0 if else. 
1. Convert the "percentage hits" to a proportion - which will be the response variable in the quantitative models. 
    + going to do this in the reverse order as it simplifies things; deal with NAs in the first step
...
```{r WGS and amplicon aggregation}
...
* the basic analysis approach here is a binomial-errors mixed-effects model. 
...
* there were 3 facets of sample complexity $\times$ 2 data subsets $\times$ 3 taxonomic levels = **18 total models**
    + set this up with a nested `for` loop; almost certainly cleaner and more elegant ways to do this
...
This analysis uses AIC to determine which dataset (amplicon vs. WGS) provides a better fit when modelling pollen grain proportion vs. read proportion. The AIC for these models will give some idea as to which dataset is more quantiatively accurate. 
...
### by taxonomic level, with markers combined
