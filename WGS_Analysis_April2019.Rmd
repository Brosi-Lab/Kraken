...
This analysis uses binomial mixed models to assess if using amplicon vs. WGS has an effect on the ability to qualitatively detect true positives vs. false negatives. This analysis is similar the qualitative mixed models to assess the effect of species richness, pollen grain proportion, etc. on the ability to qualitatively detect the correct taxa, but instead using "source" as the (sole) fixed effect. 
...
<!-- If we wanted to do the binomial analysis, we would need to re-format the dataset. For this analysis we need to have the data formatted with a separate row for each taxon present in each sample, as we do for the false negatives analysis. The issue is that to do a binomial model here, we need to have two columns, one for number of reads of that taxon and one for number of reads in that sample to other taxa. These can then be combined with `cbind` to create our response variable. While we do have data formatted in that manner, for the false positive analyses, the disconnect is that those data are formatted with all of the taxa pooled into a single row per replicate (i.e. not split out by taxon). -->

I tried specifying these models with intercept = 0. Unfortunately, we can't estimate an $R^2$ with a specification of zero intercept, at least with the `r2beta` function (not sure why). Either way, didn't do this.

<!-- those models generated *singular* fits (`boundary (singular) fit: see ?isSingular`), with near-zero estimates of variance in the nested random effect of replicate ID within mix (though ideally we would like for this estimate to be quite small!). -->

<!-- Still, we also want to compare these models to the amplicon models (below); but even without the zero-intercept specification, the amplicon models also had singular fits, also due to the near-zero estimate of variance in the nested `mix.ID/sample` random effect (and again, we want this to be a very small source of variance!). For those models, if we take out `sample` (but maintain `mix.ID` as well as a crossed random taxonomic effect), we can get non-singular fits, even *with* a zero estimate. To These are replicated below in the models ending in `nonest` (for "zero intercept"). -->
...
Pollen grain proportion was highly significantly related to read proportion across all taxonomic levels, with relatively strong $R^2$ values (except maybe for genus).

Nice to see that the two approaches (with and without the sample-replicate random effect nested within the mix) gave us essentially the exact same answers.
...
Interesting... all models show statistically significant differences between amplicon and WGS in the mean value of the residuals. But as far as I can tell from this, it looks like the amplicon data comes out ahead, i.e. the WGS / kraken data have *larger* mean residuals (worse; we want smaller). 
...
<!-- a couple of different approaches are possible here. First, we could run a linear model with proportion of sequence reads (e.g. `quant.family`) as the response variable, and as fixed effects the input `pollen.grain.proportion` and the `source` (i.e., WGS vs. amplicon) *and their interaction*. If the interaction term is significant,  -->

Run two separate linear mixed-effects models, one for WGS and one for amplicon. We already did the WGS ones above, so just have to do the ones for amplicon. Then we can ask two different things: 1) which has a better $R^2$, i.e. even if the slope is not 1 and the intercept is not zero, which one would give us a better relative estimate of pollen grain proportions; and 2) which one has a slope closer to one (which is really ultimately an analogue of the residuals analysis). While it would be sensible to constrain the intercept to be zero in both cases, again for the WGS I tried this and was not able to get $R^2$ estimates.
...
The first step here is to create a categorical `rarity` variable based on binning `pollen.grain.proportion` into four equal bins (1-25%; 26-50%; 51-75%; 76-100%). By creating a categorical variable, we can produce a figure that is highly parallel to those for the other two facets of sample complexity (species richness and relatedness)
...
Here we are putting together a figure comprised of panels we have already built in the sections for Figs 2, X, and Y. Should be very straightforward.
...
## Figure Z: true positives by all 3 complexity variables; species level
...
# fig2c = fig2c + labs(title = "A: Species Richness", subtitle = NULL)
...
* use the data Anya put together for fig 6 (`CI.krakamp.family.fp`)
...
then follow Anya's basic approach for e.g. Fig 2
...
* then (as Anya did) run `binom.confint` (from the `binomial` package) to get a mean and CI for each level
* then it's just all about graphing!
...
Here we are putting together a figure comprised of panels we have already built in the sections for Figs I, J, and K. Should be very straightforward.
