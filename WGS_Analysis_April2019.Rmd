...
This analysis uses binomial mixed models to assess if using amplicon vs. WGS approaches return different proportinos of false positive results (relative to true positives).  
...
Here the question is: do we see a statistical relationship between the proportion of pollen grains input into a sample and the proportion of read counts that are output?

two different ways to do this:

1. first do this with a linear mixed-effects model. In an ideal world, we would want the proportion of pollen grains input into a sample to map *linearly* onto the proportion of read counts that are output.
1. second, while the above is true, the data are not linear; instead they are proportional. Instead of 
