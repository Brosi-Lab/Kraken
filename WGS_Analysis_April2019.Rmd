...
# species
quant.species.lmer = lmer(quant.species ~ pollen.grain.proportion + (1|mix.ID/rep.ID) + (1|species), data = truepos.krak.species)

# genus
quant.genus.lmer = lmer(quant.genus ~ pollen.grain.proportion + (1|mix.ID/rep.ID) + (1|genus), data = truepos.krak.genus)

#family
quant.family.lmer = lmer(quant.family ~ pollen.grain.proportion + (1|mix.ID/rep.ID) + (1|family), data = truepos.krak.family)
...
coefs_quant.family <- cbind((data.frame(coef(summary(quant.family.lmer)))["pollen.grain.proportion",c("Estimate","Pr...t..")]), r2.family[2,6]) 

coefs_quant.genus <- cbind((data.frame(coef(summary(quant.genus.lmer)))["pollen.grain.proportion",c("Estimate","Pr...t..")]), r2.genus[2,6]) 

coefs_quant.species <- cbind((data.frame(coef(summary(quant.species.lmer)))["pollen.grain.proportion",c("Estimate","Pr...t..")]), r2.species[2,6]) 
...
# pull out only the relevant variables from the kraken data:
quant.krak.fam = select(truepos.krak.family, mix.ID, sample, rep.ID, family, pollen.grain.proportion, quant.family)

quant.krakamp.family = merge(

```

#### analysis for quant matching amplicon vs. WGS
