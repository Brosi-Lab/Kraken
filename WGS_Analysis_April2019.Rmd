...
The amplicon data are represented by 6 data files: two markers (ITS2 and *rbcL*) and three levels of taxonomic matching (species, genus, and family). We later format the Kraken data into three datasets representing those taxonomic levels. 

Ideally it would be best to just have it be one file with six different response columns (one for each marker and each taxonomic level), but the issue is that for different samples, matching the taxonomy at different levels leads to different numbers of rows. For example, we have two *Populus* species in someo of the samples. For matching at the species level, that is two different rows. But for genus and family, it is just one row. There are some other samples that have two different genera in the same family (e.g., *Poa* and *Zea*, both in the Poaceae). Thus, these have to be completely separate datafiles.
...
## match Kraken data to sample metadata; false negative and quantitative analyses
...
1. Add "K" to the sample names to distinguish from the samples in the amplicon data (formatted in C.7)
...
1. one consideration is that since we are aggregating all of the reads for a particular sample into one row, we have to figure out how to deal with rarity / commonness of taxa within a sample.
    + here we use the minimum % of any taxon within the sample as a measure of rarity
...
## false positives: combine WGS and amplicon data
...
One issue is that the kraken data and the 
...
# create function for later applying to the list
