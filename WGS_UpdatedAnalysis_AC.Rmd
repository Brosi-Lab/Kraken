...
The binomial mixed model yielded error messages, likely do to the very small n in the simulated dataset. 

The chi squared tests were all highly significant (p<2.2e-16), but the direction differed by taxonomic level. At the species and genus level, the simulation data had a much higher proportion of false positives than the empirical data (although, at the species level, both datasets had more false positives than true positives). However, at the family level, the simulation data had a much higher proportion of true positives than the empirical data. 
...
kable(rbcL_comp_AIC)
```
Overall, the AIC values are very small for all models. There are no obvious trends in terms of one dataset providing a better fit than another. As stated above, this does not actually answer the question of which dataset (amplicon vs. WGS) more accurrately detects presence/absence of species.
...
```
For all taxonomic levels, the AICs for the amplicon models are much smaller than for the Kraken data. This implies that the read proportion of the amplicon data is more quantitatively predictive of true pollen grain proportion. However, binomial mixed models can be used to more quantitatively answer this question (see D6.3) 
...
Most of the models gave errors, but p-values were all highly significant across taxonomic level and amplicon marker. For each test, amplicon data had a significantly lower rate of false positives/higher rate of true positives compared to the WGS data, corroborating the AIC analysis above. 