...
#Summarize the agg dataset first by source, mix.ID, and sample. Also summarize total hits for each sample, as well as sum # of false positive hits

CI.krakamp.family.fp.1 = krakamp_family_fp %>% 
select(mix.ID, sample, falsepos, source, hits) %>% group_by(source, mix.ID, sample) %>%
summarize(total.hits = sum(hits),
          fp.hits = sum(hits[falsepos=="false_pos"]))

CI.krakamp.genus.fp.1 = krakamp_genus_fp %>% 
select(mix.ID, sample, falsepos, source, hits) %>% group_by(source, mix.ID, sample) %>%
summarize(total.hits = sum(hits),
          fp.hits = sum(hits[falsepos=="false_pos"]))

CI.krakamp.species.fp.1 = krakamp_species_fp %>% 
select(mix.ID, sample, falsepos, source, hits) %>% group_by(source, mix.ID, sample) %>%
summarize(total.hits = sum(hits),
          fp.hits = sum(hits[falsepos=="false_pos"]))
