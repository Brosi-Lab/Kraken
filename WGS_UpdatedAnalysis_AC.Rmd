...
krakamp_family_fn <- as.data.frame(mapply(c, truepos.krak.family[,c("mix.ID", "family", "sample", "qual.family", "source")], amp_all[,c("mix.ID","family","sample", "qual.family", "source")]))

krakamp_genus_fn <- as.data.frame(mapply(c, truepos.krak.genus[,c("mix.ID", "genus", "sample", "qual.genus", "source")], amp_all[,c("mix.ID","genus","sample", "qual.genus", "source")]))

krakamp_species_fn <- as.data.frame(mapply(c, truepos.krak.species[,c("mix.ID", "species", "sample", "qual.species", "source")], amp_all[,c("mix.ID","species","sample", "qual.species", "source")]))
...
```
After combining rbcL and ITS2, where taxa were determined as present if they were present in rbcL OR ITS2, amplicon and WGS were not significantly different in limiting false negatives at the species level, amplicon performed better than kraken at the genus level (estimate = -1.46, p=0.001) and the model gave an error at the family level. 
...
panel.fig1 = grid.arrange(fig1a, fig1b,fig1c, ncol=1) #  heights = rep(50,3)
ggsave("fig1_combined.pdf", plot=panel.fig1, device="pdf", height=11, width = 8.5, units="in")
...
ggsave("fig2_combined.pdf", plot=fig2, device="pdf", width=169, units="mm")
...
fig3_sub_family=truepos.krak.family%>%
  filter(question.1==2|question.2==2|question.3==2) %>%
...
fig3=grid.arrange(fig3a,fig3b,fig3c, nrow=2)
...
ggsave("fig3_combined.pdf", plot=panel.fig3, device="pdf", height=8.5, width = 11, units="in")
```
...
#Figure 4 Panel
ggsave("WGS_fig4a.pdf", fig6a, device="pdf", width=7, height=5, units="in")
ggsave("WGS_fig4b.pdf", fig6b, device="pdf", width=7, height=5, units="in")
ggsave("WGS_fig4c.pdf", fig6c, device="pdf", width=7, height=5, units="in")

# save panel
panel.fig4 = grid.arrange(fig4a, fig4b, fig4c,ncol=1) #  heights = rep(50,3)
ggsave("WGS_fig4_combined.pdf", plot=panel.fig4, device="pdf", height=11, width = 11, units="in")
```
