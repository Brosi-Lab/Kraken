...
krakamp_its_family = rbind(amp_its_family_reshape, as.data.frame(agg.krak.family))
krakamp_its_genus = rbind(amp_its_genus_reshape, as.data.frame(agg.krak.genus))
krakamp_its_species = rbind(amp_its_species_reshape, as.data.frame(agg.krak.species))
krakamp_rbc_family = rbind(amp_rbc_family_reshape, as.data.frame(agg.krak.family))
krakamp_rbc_genus = rbind(amp_rbc_genus_reshape, as.data.frame(agg.krak.genus))
krakamp_rbc_species = rbind(amp_rbc_species_reshape, as.data.frame(agg.krak.species))
    ...
    for(k in 1:3){ # 'taxon': species, genus, family
      for(l in 1:2) { # 'datasubset': sub or all
        ...
        mixed = paste(namer, " = suppressWarnings(glmer(qual.", taxon[k],
          " ~ ",  question[q], " + (1|mix.ID/sample/rep.ID) + (1|", taxon[k], "), family = binomial, 
