...
1. Qualitative matching 
    ...
    b. the key bit is to aggregate the counts---for each sample-by-species combination---of whether or not a true positive was detected, merged into a single file for both the amplicon data and the Kraken data, with a column added for `sample.source` etc. *One key part of doing this will be to exclude samples from the amplicon data which do not have a corresponding Kraken data point*---we want all samples to be "paired".
    b. **false positive analysis**---*for the mixed-amplicon paper, this was not analyzed formally; false positives are essentially ubiquitous so a formal presence / absence analysis may not be informative*. See below for a quantitative analysis of the proportions of false positives in a sample, following the point immediately above. 
    c. when we go to do the `dada2` vs. `QIIME` analysis, this false-positive analysis will be particularly key.
...
2. a direct comparison of how the *amplicon-based* results compare to the whole-genome shotgun results.
    ...
    b. another possibility is to compare the two models with AIC... but... the data are different so I am thinking this will not work? it's tricky as the 'true data'---the samples used for both---are the same...
...
```{r data import}
krak = read.csv("/Users/ajcutle/Kraken/kraken.csv")
mixes = read.csv("/Users/ajcutle/Kraken/pollen-mixes-proportions.csv")
simkrak = read.csv("/Users/ajcutle/Kraken/kraken_sim.csv")
...
Run a GLMM for this comparison. 
